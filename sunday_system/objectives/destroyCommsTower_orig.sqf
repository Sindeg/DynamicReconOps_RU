...
if (!isNil "_transmitter") then {
	...
	_markerPower setMarkerText "Вышка связи";
	...
	_taskDesc =	format ["Выведите из строя связь %1 чтобы лишить их возможности вызывать подкрепления. По нашим данным, большой поток информации в %3 передается через %2.", enemyFactionName, _transmitterName, aoName];	
	_task = ["commsTask_orig", true, [_taskDesc, _taskTitle, ""], _transmitter, "CREATED", 0, true, true, "danger", true] call BIS_fnc_setTask;		
	...
	[_transmitter] spawn {
		...
		while {alive _transmitter} do {
			...
			if (!alive _transmitter) then {				
				["commsTask_orig", "SUCCEEDED", true] spawn BIS_fnc_taskSetState;
